package com.rechargeitnow.pageObject;

public class SummaryPageWapObject {
	
	public static String paymentOptionText_Xpath = "//*[@id='paymentoption']/h2/span";
	public static String proceedSummaryButton_Xpath = "//input[@value='Proceed']";
	public static String promoDiscount_Xpath = "//*[@id='paymentform']/div[1]/ul/li[5]";
	public static String RINcashPoint_xpath = "//*[@id='paymentform']/div[1]/ul/li[6]";
	public static String PDCAmountText_Xpath = "//*[@id='paymentform']/div[1]/ul/li[5]";
	public static String discountAmountText_Xpath = "//*[@id='paymentform']/div[1]/ul/li[6]";
	public static String RINcashAmountText_Xpath = "//*[@id='paymentform']/div[1]/ul/li[7]";
	public static String netPayableAmount_Xpath = "//*[@id='paymentform']/div[1]/ul/li[4]";

}
